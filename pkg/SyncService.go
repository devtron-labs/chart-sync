...
func (impl *SyncServiceImpl) syncRepo(repo *sql.ChartRepo) error {
	...
	for name, chartVersions := range indexFile.Entries {
		...
		err := impl.updateChartVersions(id, &chartVersions, repo.Url, repo.Username, repo.Password, repo.SecuredWithTls)
		if err != nil {
...
func (impl *SyncServiceImpl) updateChartVersions(appId int, chartVersions *repo.ChartVersions, baseurl string, username string, password string, secureWithTls bool) error {
	...
	for _, chartVersion := range *chartVersions {
		...
		rawValues, readme, valuesSchemaJson, notes, err := impl.helmRepoManager.ValuesJson(baseurl, chartVersion, username, password, secureWithTls)
		if err != nil {
